...
#define Z_PINCTRL_SILABS_MODE_INIT(node)                                       \
	(DT_PROP(node, drive_push_pull)                                        \
	 ? (4 + DT_PROP(node, silabs_alternate_port_control))                  \
	 : DT_PROP(node, drive_open_source)                                    \
	 ? (6 + DT_PROP(node, bias_pull_down))                                 \
	 : DT_PROP(node, drive_open_drain)                                     \
	 ? (8 + DT_PROP(node, silabs_input_filter)                             \
	    + 2 * DT_PROP(node, bias_pull_up)                                  \
	    + 4 * DT_PROP(node, silabs_alternate_port_control))                \
...
#define Z_PINCTRL_SILABS_DOUT_INIT(node)                                       \
	(DT_PROP(node, drive_push_pull)                                        \
	 ...
	 : DT_PROP(node, input_disable)                                        \
	 ? DT_PROP(node, bias_pull_up)                                         \
	 : 0)

#define Z_PINCTRL_STATE_PIN_INIT(node, prop, idx)                              \
	{                                                                      \
		.base_offset  = FIELD_GET(SILABS_PINCTRL_PERIPH_BASE_MASK,     \
					  DT_PROP_BY_IDX(node, prop, idx)),    \
		.port         = FIELD_GET(SILABS_PINCTRL_GPIO_PORT_MASK,       \
					  DT_PROP_BY_IDX(node, prop, idx)),    \
		.pin          = FIELD_GET(SILABS_PINCTRL_GPIO_PIN_MASK,        \
					  DT_PROP_BY_IDX(node, prop, idx)),    \
		.en_bit       = (FIELD_GET(SILABS_PINCTRL_HAVE_EN_MASK,        \
					   DT_PROP_BY_IDX(node, prop, idx))    \
				 ? FIELD_GET(SILABS_PINCTRL_EN_BIT_MASK,       \
					     DT_PROP_BY_IDX(node, prop, idx))  \
				 : 0xFF),                                      \
		.route_offset = FIELD_GET(SILABS_PINCTRL_ROUTE_MASK,           \
					  DT_PROP_BY_IDX(node, prop, idx)),    \

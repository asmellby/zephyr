...
static int wdt_silabs_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	...
	timeout_cycles = timeout * data->clock_rate / MSEC_PER_SEC;
	if (!IN_RANGE(timeout_cycles, 0x9, 0x40001)) {
	...
	window_cycles = ROUND_DOWN(window * data->clock_rate / MSEC_PER_SEC,
				   timeout_cycles / WINDOW_FRACTION);
	...
	warn_cycles =
		ROUND_DOWN(warn * data->clock_rate / MSEC_PER_SEC, timeout_cycles / WARN_FRACTION);
